...
<section class="grid gap-6 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 max-w-2xl">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Add Locker</h2>
            <p class="text-sm text-gray-500 mt-0.5">Create a new locker for a location.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('lockers.store') }}" class="grid gap-5">
        @csrf
        ...
        {{-- Password --}}
        <div class="grid gap-1.5">
            <label for="password" class="text-sm font-medium text-gray-700">Locker Password</label>
            <div class="relative">
                <svg class="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
